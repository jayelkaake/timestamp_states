  ...
  describe "scopes" do
    ...
    describe "#set_timestamp_state" do
      it "returns the expected models within the date range" do
        expect {
          model.install
        }.to change(model, :installed_at).from(nil)
                                         .and change(model, :installed?).from(false).to(true)
                                                                        .and change(model, :not_installed?).from(true).to(false)

        expect(model.installed_at).to be_within(1.second).of(Time.now.utc)

        expect {
          model.uninstall
        }.to change(model, :installed_at).to(nil)
                                         .and change(model, :installed?).from(true).to(false)
                                                                        .and change(model, :not_installed?).from(false).to(true)
      end
    end
  end
  ...
  describe "callbacks" do
    describe "after_timestamp_change" do
      it "triggers the callbacks once" do
        ...
        model.singleton_class.around_install -> (_, process) do
